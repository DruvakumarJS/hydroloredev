      ...
      <table class="table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td width="100px">{{date('d M Y',strtotime($value->installation_date))}}</td>
            <td>{{$value->no_of_channels}}</td>
            <td>{{$value->crops_name}}</td>
            <td>{{$value->require_monitoring}}</td>
            <td>{{$value->comments}}</td>
            <td>
               <a href="{{route('edit_enquiry',$value->id)}}"><button class="btn btn-sm btn-outline-success">Edit</button></a>
            </td>
            <td>
